...
class YOLO:
    ...
    def get_bounding_boxes(self, frame_dict, suspect_data, image_original, suspect_img_encoded, suspect_feature1_encoded):
        ...
        if self.camId is not None and self.new_bounding_box_cord is not None:
            ...
            self.do_ReID = False
            status = 'new_position'
            target_gps_data.append(meta_extractor.get_metadata_by_camera_id(str(self.camId)))
...
class CameraWidget(QWidget):
        ...
        while True:
            try:
                if self.capture.isOpened() and self.online:
                    ...
                    self.save_fps_data(self._fps_data)
    ...
    def set_frame(self):
        ...
        if self.deque and self.online:
            ...
            cv2.putText(self.frame, FPS, (0, 40),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), lineType=cv2.LINE_AA)
    ...
    def save_fps_data(self, data):
        path = self.results_path + str(self.frame_id) + '_fps_data.txt'
        feed_fps = str(int(np.mean(data)))
        print(self.frame_id,',FPS:', feed_fps)
    ...
    def showImage(self, stream_id):
        if True:
            ...
            target_gps_data.append(meta_extractor.get_metadata_by_camera_id(str(stream_id)))
...
if __name__ == '__main__':
    ...
    for i, url in enumerate(urls):
        cam = CameraWidget(screen_width // 4, screen_height // 4, url, id=i)
        window.ml.addWidget(cam.get_video_frame()[0], i // 4, i % 4, 1, 1)
        cams.append(cam)
